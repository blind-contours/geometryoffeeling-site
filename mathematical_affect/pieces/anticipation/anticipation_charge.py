"""
Geometry of Feeling — Anticipation: Anticipation Charge
...
def render():
    ...
    ceiling=PAD_B+PH*0.92
    n=22
    for i in range(n):
        frac=i/(n-1)
        RC=0.10+frac*0.50  # time constant
        V0=PH*(0.60+frac*0.28)
        ys=PAD_B+PH*0.06+V0*(1-np.exp(-t/RC))
        ys=np.minimum(ys,ceiling)
        if frac<0.3: col=DAWN_BLUE
        elif frac<0.6: col=LILAC
        else: col=APRICOT
        alpha=0.08+0.45*(1-abs(frac-0.5)*1.3)
        lw=0.4+1.1*(1-abs(frac-0.5))
        draw_lc(ax,xs,ys,col,lw=lw,alpha=alpha,zo=3,smooth=3)
    # ceiling — what they're all reaching for
    ax.plot([PAD_L,PAD_L+PW],[ceiling,ceiling],
            color=rgba(PALE_GOLD,0.18),linewidth=0.8,linestyle='--',zorder=5)
    label(ax,"V(t)=V\u2080(1\u2212e^(\u2212t/RC))")
    save(fig,"anticipation_charge.pdf")
