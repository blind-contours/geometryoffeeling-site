"""
Geometry of Feeling — Cycles: Cycles Phase Portrait
...
# ═══════════════════════════════════════════════════════════════
# 3. PHASE PORTRAIT — Van der Pol, more trajectories
# ═══════════════════════════════════════════════════════════════
def render():
    fig,ax=make_fig()
    n_traj=24
    ...
    for i in range(n_traj):
        frac=i/(n_traj-1)
        r0=0.2+frac*4.5
        angle0=2*np.pi*frac*1.3
        x=r0*np.cos(angle0)
        v=r0*np.sin(angle0)
        dt=0.004; n_steps=12000
        xs_p=[]; ys_p=[]
        for _ in range(n_steps):
            xs_p.append(x); ys_p.append(v)
            x_new=x+v*dt
            v_new=v+(mu*(1-x**2)*v-x)*dt
            x=x_new; v=v_new
        xs_p=np.array(xs_p); ys_p=np.array(ys_p)
        xs_c=cx+PW*0.20*xs_p
        ys_c=cy+PH*0.14*ys_p
        if frac<0.25: col=WINTER
        elif frac<0.50: col=SPRING
        elif frac<0.75: col=SUMMER
        else: col=AUTUMN
        alpha=0.12+0.65*(1-abs(frac-0.5)*1.3)
        lw=0.42+1.12*(1-abs(frac-0.5))
        draw_lc_xy(ax,xs_c,ys_c,col,lw=lw,alpha=alpha,zo=3)
    label(ax,"x''\u2212\u03bc(1\u2212x\u00b2)x'+x=0")
    save(fig,"cycles_phase_portrait")
