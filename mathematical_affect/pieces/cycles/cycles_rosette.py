"""
Geometry of Feeling — Cycles: Cycles Rosette
...
# ═══════════════════════════════════════════════════════════════
# 9. ROSETTE — rose curves r=cos(n*theta/m)
# ═══════════════════════════════════════════════════════════════
def render():
    fig,ax=make_fig()
    configs=[(2,1),(3,1),(5,2),(4,3),(7,3),(5,3),(8,3),(7,4)]
    cols=[SPRING,SUMMER,AUTUMN,WINTER,BLOSSOM,FROST,HARVEST,RENEWAL]
    for i,(n,m) in enumerate(configs):
        frac=i/(len(configs)-1)
        theta=np.linspace(0,2*np.pi*m,6000)
        r=PH*(0.15+frac*0.30)*np.cos(n*theta/m)
        xs_r=cx+r*1.3*np.cos(theta)
        ys_r=cy+r*np.sin(theta)
        col=cols[i]
        alpha=0.14+0.65*(1-abs(frac-0.5)*1.3)
        lw=0.49+1.4*(1-abs(frac-0.5))
        draw_lc_xy(ax,xs_r,ys_r,col,lw=lw,alpha=alpha,zo=3+i)
    label(ax,"r=cos(n\u03b8/m)")
    save(fig,"cycles_rosette")
