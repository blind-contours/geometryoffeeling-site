"""
Geometry of Feeling — Cycles: Cycles Pulse Train
...
# ═══════════════════════════════════════════════════════════════
# 19. PULSE TRAIN — repeating peaked waveforms
# ═══════════════════════════════════════════════════════════════
def render():
    fig,ax=make_fig()
    ...
    for li in range(n_lines):
        ...
        period=0.06+0.08*frac
        centers=np.arange(0,1+period,period)
        signal=np.zeros_like(t)
        sigma=period*0.15
        for c in centers:
            signal+=np.exp(-0.5*((t-c)/sigma)**2)

        y_base=PAD_B+PH*(0.06+0.88*frac)
        amplitude=PH*0.025*(1+0.3*(1-frac))
        ...
        draw_lc(ax,xs,ys,col,lw=lw,alpha=alpha,zo=3+li)

    label(ax,"f(t)=\u03a3 e^(\u2212(t\u2212nT)\u00b2/2\u03c3\u00b2)")
    save(fig,"cycles_pulse_train")
