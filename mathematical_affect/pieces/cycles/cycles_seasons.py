"""
Geometry of Feeling — Cycles: Cycles Seasons
...
def render():
    ...
    for si,(col,phase) in enumerate(season_data):
        for li in range(n_layers):
            frac=li/(n_layers-1)
            # envelope: one full cycle across canvas
            envelope=np.sin(2*np.pi*t+phase)
            envelope=np.clip(envelope,0,1)  # only positive portion
            # individual waveforms within envelope
            freq=8+li*4
            signal=envelope*np.sin(freq*2*np.pi*t+li*0.5+si*1.2)
            y_base=PAD_B+PH*0.5
            amplitude=PH*0.35
            ys=y_base+amplitude*signal
            alpha=0.10+0.35*(1-frac*0.6)
            lw=0.4+1.2*(1-frac*0.5)
            draw_lc(ax,xs,ys,col,lw=lw,alpha=alpha,zo=3+si*n_layers+li)

    label(ax,"f\u2096(t)=sin(\u03c9t+k\u03c0/2)\u00b7sin(n\u03c9t)")
    save(fig,"cycles_seasons")
