"""
Geometry of Feeling — Surrender: Surrender Terminal
...
def render():
    ...
    for i in range(n):
        frac = i / (n - 1)
        xs_t = PAD_L + PW * t
        v_terminal = PAD_B + PH * (0.10 + frac * 0.80)
        g_rate = 3 + frac * 8
        ys_t = v_terminal * np.tanh(g_rate * t)
        # shift so curves start from bottom-left and approach their asymptote
        ys_t = PAD_B + PH * 0.05 + (v_terminal - PAD_B - PH * 0.05) * np.tanh(g_rate * t)
        if frac < 0.25: col = DARK
        elif frac < 0.5: col = MED_DARK
        elif frac < 0.75: col = SOFT
        else: col = WARM
        alpha = 0.12 + 0.50 * (1 - abs(frac - 0.5) * 1.3)
        lw = 0.5 + 1.1 * (1 - abs(frac - 0.5))
        draw_lc(ax, xs_t, ys_t, col, lw=lw, alpha=alpha, zo=3)
    label(ax, "v(t)=v_t\u00b7tanh(gt/v_t)")
    save(fig, "surrender_terminal")
