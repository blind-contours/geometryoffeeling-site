"""
Geometry of Feeling — Connection: Connection Weave
...
def render():
    ...
    for i in range(6):
        ...
        freq = 5 + i*2; amp = PH*0.05
        ys1 = y_base + amp*np.sin(2*np.pi*freq*t)
        ys2 = y_base + amp*np.sin(2*np.pi*freq*t + np.pi/2)
        c1 = [ROSE_GOLD, WARM_ROSE, SOFT_ROSE][i%3]
        c2 = [GOLD, PALE_GOLD, AMBER][i%3]
        draw_lc(ax, xs, ys1, c1, lw=1.2, alpha=0.42, zo=4)
        draw_lc(ax, xs, ys2, c2, lw=1.2, alpha=0.42, zo=5)
    label(ax, "y\u2081=sin(\u03c9t), y\u2082=sin(\u03c9t+\u03c0/2)")
    save(fig, "connection_weave.pdf")
