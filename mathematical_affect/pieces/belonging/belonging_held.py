"""
Geometry of Feeling — Belonging: Held
Two distinct forms: a sage sling holding a warm kidney-shaped form.
The asymmetry of care — one shape cradles another, and the held form
settles into the space made for it.

Source: cradle_v1 from R3f.
"""
...
def _held_field(X, Y, cx, cy, a, b, ring_freq=3.0, ring_depth=0.7,
                ring_contrast=0.35, falloff=1.5, warp_amp=0.35, warp_seed=0):
    ...
    base = 1.0 - ring_contrast
    field = glow * (base + ring_contrast * rings)
    return field, glow
...
def render():
    ...
    X_s, Y_s = _domain_warp(X, Y, amp=0.22, seed=901)

    # HOLDER (sling): sage, centered above
    sling_cx = ART_CX - 0.10
    sling_cy = ART_CY + 0.15
    sling = _gauss_xy(X_s, Y_s, sling_cx, sling_cy, sx=3.2, sy=2.2) * mask

    # HELD: offset down-right, kidney shape
    held_cx = ART_CX + 0.20
    held_cy = ART_CY - 0.35
    f_held, glow_held = _held_field(
        X, Y, held_cx, held_cy,
        a=1.5, b=1.2,
        ring_freq=4.5, ring_depth=1.2, ring_contrast=0.50,
        falloff=1.5, warp_amp=0.35, warp_seed=903)
    f_held = f_held * mask
    glow_held = glow_held * mask

    # Sling body
    render_mass(ax, sling, ext,
                colors=[BG, SAGE, SAGE_LIGHT, SAND, CREAM_WARM],
                alpha_gamma=0.52, alpha_max=0.68, zorder=2)
    render_fbm_layer(ax, X, Y, ext, sling, SAGE, 0.16,
                     seed=911, base_sigma=55, zorder=2.5)

    # Blue membrane
    render_blue_membrane(ax, X, Y, ext,
        outer_field=sling, inner_field=glow_held ** 0.80,
        blue_colors=[BG, MEMBRANE_BLUE_LT, MEMBRANE_BLUE, DUSTY_BLUE],
        alpha_max=0.55, gamma=0.50, fbm_seed=913, zorder=3)

    # Held form — warm, no pink/rose
    render_mass(ax, f_held, ext,
                colors=[BG, SAGE, SAND, HONEY, APRICOT, CREAM_WARM],
                alpha_gamma=0.50, alpha_max=0.92, zorder=4)

    # FBM texture
    focal = _gauss(X, Y, held_cx, held_cy, sig=1.8)
    render_fbm_layer(ax, X, Y, ext, glow_held, HONEY, 0.22,
                     seed=915, base_sigma=42, zorder=5, focal_mask=focal)
    render_fbm_layer(ax, X, Y, ext, glow_held, SAND, 0.10,
                     seed=917, base_sigma=34, zorder=5.1, focal_mask=focal)
...
if __name__ == '__main__':
    ...
    pdf_path = os.path.join(OUTPUT_DIR, "belonging_held.pdf")
    fig.savefig(pdf_path, format='pdf', facecolor=BG)

    os.makedirs(PRINT_DIR, exist_ok=True)
    jpg_path = os.path.join(PRINT_DIR, "belonging_held.jpg")
