"""
Geometry of Feeling — Belonging: Warmth
A large warm cell filling most of the frame — immersive warmth with a blue
membrane boundary. Like looking at a cell so close it fills your vision.
The primal unit of belonging: if you have a cell, you are alive.

Source: enfold_v1b from R3b.
"""
...
def render():
    ...
    X_w, Y_w = _domain_warp(X, Y, amp=0.25, seed=91)
    outer = _gauss_xy(X_w, Y_w, cell_cx, cell_cy, sx=5.2, sy=3.8) * mask
    inner = _gauss_xy(X_w, Y_w, cell_cx, cell_cy, sx=4.0, sy=2.9) * mask
    nucleus = _gauss_xy(X, Y, nuc_cx, nuc_cy, sx=1.6, sy=1.3) * mask

    render_blue_membrane(ax, X, Y, ext,
        outer_field=outer, inner_field=inner ** 0.85,
        blue_colors=[BG, MEMBRANE_BLUE_LT, MEMBRANE_BLUE, DUSTY_BLUE],
        alpha_max=0.68, gamma=0.48, fbm_seed=601, zorder=2)

    render_mass(ax, inner, ext,
                colors=[BG, SAND, HONEY, ROSE_DUST, TERRACOTTA],
                alpha_gamma=0.50, alpha_max=0.85, zorder=3)

    render_mass(ax, nucleus, ext,
                colors=[BG, TERRACOTTA, DEEP_SIENNA, DEEP_BURNT, DEEP_CRIMSON],
                alpha_gamma=0.38, alpha_max=0.95, zorder=4)

    focal = _gauss(X, Y, cell_cx, cell_cy, sig=3.0)
    render_fbm_layer(ax, X, Y, ext, inner, "#F0D8B0", 0.35,
                     seed=603, base_sigma=55, zorder=4.5, focal_mask=focal)
    render_fbm_layer(ax, X, Y, ext, nucleus, DEEP_WARM, 0.25,
                     seed=605, base_sigma=40, zorder=4.6, focal_mask=focal)

    hot = _gauss(X, Y, nuc_cx, nuc_cy, sig=0.65) * mask
    render_mass(ax, hot, ext,
                colors=[BG, HONEY, EMBER],
                alpha_gamma=0.34, alpha_max=0.40, zorder=4.7)
...
if __name__ == '__main__':
    ...
    pdf_path = os.path.join(OUTPUT_DIR, "belonging_warmth.pdf")
    fig.savefig(pdf_path, format='pdf', facecolor=BG)

    os.makedirs(PRINT_DIR, exist_ok=True)
    jpg_path = os.path.join(PRINT_DIR, "belonging_warmth.jpg")
