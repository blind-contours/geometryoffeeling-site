"""
Geometry of Feeling — Peace: Peace Breath
...
def render():
    fig, ax = make_fig()
    n = 18
    cols = [DEEP_SAGE, LICHEN, MOSS, FERN, OCEAN, WATER, MINERAL, STONE, DUSK]
    for i in range(n):
        frac = i / (n - 1)
        t = np.linspace(0, 1, 1500)
        xs = PAD_L + PW * t
        # Breathing: slow sinusoid with varying amplitude
        y_base = PAD_B + PH * (0.08 + frac * 0.84)
        freq = 1.5 * np.pi  # one full breath cycle across the canvas
        amp = PH * 0.05 * (0.3 + 0.7 * np.sin(np.pi * frac))
        ys = y_base + amp * np.sin(freq * t + frac * 0.5)
        col = cols[i % len(cols)]
        alpha = 0.77 + 0.17 * np.sin(np.pi * frac)
        lw = 1.12 + 0.84 * np.sin(np.pi * frac)
        draw_lc(ax, xs, ys, col, lw=lw, alpha=alpha, zo=3, smooth=5)
    label(ax, "A(t)=A\u2080\u00b7sin(2\u03c0t/T)")
    save(fig, "peace_breath.pdf")
