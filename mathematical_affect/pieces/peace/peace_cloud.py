"""
Geometry of Feeling — Peace: Peace Cloud
...
def render():
    ...
    n = 12
    cols = [FOG_COL, STONE, SKY, WATER, MINERAL, WARM_GREY, OCEAN, DUSK,
            DEEP_SAGE, LICHEN, CLAY, CEDAR]
    np.random.seed(22)
    for i in range(n):
        frac = i / (n - 1)
        y_base = PAD_B + PH * (0.10 + frac * 0.78)
        ys = np.full_like(t, y_base)
        n_bumps = 3 + i % 3
        for k in range(n_bumps):
            mu = np.random.uniform(0.1, 0.9)
            sigma = np.random.uniform(0.06, 0.15)
            amp = PH * np.random.uniform(0.015, 0.045)
            ys = ys + amp * np.exp(-0.5 * ((t - mu) / sigma) ** 2)
        col = cols[i]
        alpha = 0.40 + 0.15 * np.sin(np.pi * frac)
