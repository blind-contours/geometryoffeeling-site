"""
Geometry of Feeling — Peace: Peace Sand
...
def render():
    ...
    n = 35
    cols = [CLAY, SAND_COL, WARM_GREY, STONE, EARTH, CEDAR, FOG_COL, MINERAL, RUST, BARK]
    for i in range(n):
        frac = i / (n - 1)
        y_base = PAD_B + PH * (0.04 + frac * 0.92)
        epsilon = PH * 0.012 * (0.5 + 0.5 * np.sin(np.pi * frac))
        omega = 2 * np.pi * (0.7 + i * 0.12)
        phase = i * 0.6
        ys = y_base + epsilon * np.sin(omega * t + phase)
        col = cols[i % len(cols)]
        alpha = 0.60 + 0.26 * np.sin(np.pi * frac)
        lw = 0.84 + 0.56 * np.sin(np.pi * frac)
        draw_lc(ax, xs, ys, col, lw=lw, alpha=alpha, zo=3, smooth=5)
    label(ax, "y_n(t)=y_n+\u03b5\u00b7sin(\u03c9t+\u03c6_n)")
    save(fig, "peace_sand.pdf")
