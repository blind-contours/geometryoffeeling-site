"""
Geometry of Feeling — Comprehending: Depth

The center wing of the Comprehension triptych — bronze and patinated teal.
Three vortices in a horizontal row sweep the eye left to right across the
upper field. Streamlines reach down deep into the chunky block layer so
the storm and the rest-state interpenetrate — erosion rather than separation.
Two narrow piers at the outer margins frame the movement without arresting
it. The center is "Wave Train": comprehension as sustained sweep, not
momentary fixation.

Mechanism (same v9 grammar as the left panel):
  - Recursive feature-driven Mondrian subdivision driven by scalar field
    gradients. Vorticity-rich regions get more cuts (here biased LOW so
    blocks intrude into the vortex zones — the erosion effect).
  - Pier injection: tall vertical strips reach up from the block layer
    into the storm zone, shallow-depth so they read as monumental piers.
  - Soft horizon: depth gradient top→bottom (no clean horizon line).
  - Vertical aspect bias band in the middle prefers vertical splits.
  - Velocity-tinted per-cell quantization.
  - Extended streamlines: flow_top_y lowered so lines reach into blocks.

This script reproduces the exact configuration of "P4 — Wave Train"
from scripts/wonder_comprehension_bronze_center.py.
"""
import os
...
# ─── Configuration: C3 — Convergence ────────────────────────────────
# Bronze + patinated teal palette. Twin vortices of nearly equal weight,
# resolution in the space between. Reproduces comp_center_v2_03.jpg from
# scripts/wonder_comprehension_bronze_center.py.

PAL_BRONZE = [
    '#0a1418', '#16282c', '#26403e', '#3e4e42', '#6a5836',
    '#957a42', '#bfa060', '#dcc488', '#ede0b4', '#f5ecd2',
]
...
CONFIG = {
    ...
    'field_scale': 5.0,
    'complexity': 0.9, 'offset': 1.7, 'bg_amp': 0.16,
    # P4 — Wave Train: three vortices in a horizontal row across the
    # upper field, eye sweeps left to right
    'v1_pos': (0.95, 2.85), 'v1_strength': 1.35, 'v1_radius': 0.26,
    'v2_pos': (2.50, 2.95), 'v2_strength': 1.30, 'v2_radius': 0.26,
    'v3_pos': (4.10, 2.80), 'v3_strength': 1.30, 'v3_radius': 0.26,
    # Soft horizon
    'max_depth_top': 10, 'max_depth_bot': 5,
    'depth_power': 0.95, 'min_cell': 3,
    'edge_margin': 0.22, 'split_jitter': 0.10,
    # Erosion: low vort_depth_bias so blocks intrude into vortex zones
    'vort_depth_bias': 0.6,
    'velocity_tint': 0.58,
    # Vertical aspect bias band
    'vbias_top': 0.35, 'vbias_bot': 0.78, 'vbias_strength': 0.65,
    # Pier injection — two narrow outer piers framing the wave train
    'pier_xs': [0.18, 0.82], 'pier_widths': [0.10, 0.10],
    'pier_y_top': 0.22, 'pier_depth': 3,
    # Grid hairlines
    'grid_color': '#0a1418', 'grid_lw': 0.35, 'grid_alpha_max': 0.32,
    'grid_lw_range': 0.65, 'grid_min_area_norm': 0.0008,
    # Flow lines — bronze cream / patina mid, extended down into blocks
    'n_flow_lines': 1700, 'flow_steps': 250, 'flow_step_size': 0.0035,
    'flow_max_alpha': 0.55, 'flow_max_lw': 0.95, 'flow_top_y': 0.18,
    'flow_color_top': '#f4ecd2', 'flow_color_mid': '#7a6838',
    'glow_alpha_mult': 0.20, 'glow_width_mult': 3.8,
    # Wave train focal seeding — eye sweeps across three attractors
    'focal_bias': 0.30, 'secondary_bias': 0.35,
    'seed': 2344,
...
if __name__ == '__main__':
    print("═══ Comprehending: Depth ═══")

    fig = render(CONFIG)
    pdf_path = os.path.join(OUTPUT_DIR, "comprehending_depth.pdf")
    fig.savefig(pdf_path, format='pdf', facecolor=MARGIN_COLOR, dpi=DPI)
    print(f"  saved {pdf_path}")
    jpg_path = os.path.join(PRINT_DIR, "comprehending_depth.jpg")
